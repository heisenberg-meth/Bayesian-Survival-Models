...
class SurvivalDataPipeline:
    ...
    def __init__(
        ...
        ordinal_cols: dict[str, list[Any]] | None = None,
        random_state: int = 42,
    ):
    ...
    def run(
        ...
        validator = DatasetValidator(
            raw_df, self.dataset_name, self.time_col, self.event_col
        )
        val_report = validator.validate()

        if not val_report["passed"]:
            raise ValueError(
                f"Dataset validation failed for {self.dataset_name}: {val_report['errors']}"
            )
        ...
        metadata = {
            ...
            "train_censoring_rate": float(
                round((1 - train_processed["event"].mean()) * 100, 2)
            ),
            "val_censoring_rate": float(
                round((1 - val_processed["event"].mean()) * 100, 2)
            ),
            "test_censoring_rate": float(
                round((1 - test_processed["event"].mean()) * 100, 2)
            ),
    ...
    def _fit_transform_features(
        ...
        num_cols = [
            ...
            if c not in cat_cols and c not in self.ordinal_cols
        ]
        ...
        for ord_col, ord_order in self.ordinal_cols.items():
            if ord_col in X_train.columns:
                ...
                encoded_train_parts.append(
                    pd.DataFrame(
                        {ord_col: X_train[ord_col].map(ord_map).fillna(0)},
                        index=X_train.index,
                    )
                )
                encoded_val_parts.append(
                    pd.DataFrame(
                        {ord_col: X_val[ord_col].map(ord_map).fillna(0)},
                        index=X_val.index,
                    )
                )
                encoded_test_parts.append(
                    pd.DataFrame(
                        {ord_col: X_test[ord_col].map(ord_map).fillna(0)},
                        index=X_test.index,
                    )
                )
        ...
        for cat_col in cat_cols:
            ...
            for d_cat in dummy_cats:
                ...
                encoded_train_parts.append(
                    pd.DataFrame(
                        {col_name: train_dummies.get(col_name, 0).astype(float)},
                        index=X_train.index,
                    )
                )
                encoded_val_parts.append(
                    pd.DataFrame(
                        {col_name: val_dummies.get(col_name, 0).astype(float)},
                        index=X_val.index,
                    )
                )
                encoded_test_parts.append(
                    pd.DataFrame(
                        {col_name: test_dummies.get(col_name, 0).astype(float)},
                        index=X_test.index,
                    )
                )
        ...
        continuous_cols = [
            c for c in df_train_enc.columns if df_train_enc[c].nunique() > 2
        ]
