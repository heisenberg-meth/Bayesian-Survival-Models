...
class DatasetValidator:
    ...
    def validate(self) -> Dict[str, Any]:
        ...
        if self.time_col not in columns:
            report["passed"] = False
            report["errors"].append(f"Target time column '{self.time_col}' missing from dataset.")

        if self.event_col not in columns:
            report["passed"] = False
            report["errors"].append(f"Target event column '{self.event_col}' missing from dataset.")
        ...
        if invalid_time_count > 0:
            report["passed"] = False
            report["errors"].append(f"Found {invalid_time_count} non-positive (<=0) survival time values in '{self.time_col}'.")

        null_time_count = time_vals.isnull().sum()
        if null_time_count > 0:
            report["passed"] = False
            report["errors"].append(f"Found {null_time_count} missing values in target time column '{self.time_col}'.")
        ...
        if not unique_events.issubset({0, 1, 0.0, 1.0}):
            report["passed"] = False
            report["errors"].append(f"Target event column '{self.event_col}' contains invalid non-binary values: {unique_events}.")

        null_event_count = event_vals.isnull().sum()
        if null_event_count > 0:
            report["passed"] = False
            report["errors"].append(f"Found {null_event_count} missing values in target event column '{self.event_col}'.")
        ...
        for col in columns:
            ...
            if self.df[col].dtype in ['int64', 'float64']:
                ...
                if inf_count > 0:
                    report["passed"] = False
                    report["errors"].append(f"Column '{col}' contains {inf_count} infinite (inf) values.")
        ...
        report["checks"]["censoring_rate"] = float(round((report["checks"]["censored_count"] / len(self.df)) * 100, 2))
