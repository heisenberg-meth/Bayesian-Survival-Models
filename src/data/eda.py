...
class EDAAnalyzer:
    def __init__(
        self, filepath, dataset_name, time_col, event_col, categorical_cols=None
    ):
    ...
    def dataset_integrity(self):
        ...
        for col in self.headers:
            ...
            if not non_nulls:
                dtypes[col] = "empty"
            elif col in self.user_categorical_cols or any(
                isinstance(x, str) for x in non_nulls
            ):
        ...
        clinical_info = {
            "GBSG2": {
                "horTh": (
                    "Hormone Therapy",
                    "Categorical (yes/no)",
                    "Hormone therapy status (tamoxifen treatment)",
                ),
                "age": ("Age", "Numerical (Years)", "Patient age at diagnosis"),
                "menostat": (
                    "Menopausal Status",
                    "Categorical (Pre/Post)",
                ...
                "cens": (
                    "Recurrence Event",
                    "Binary Target (0/1)",
                    "Recurrence indicator (1=event/recurrence, 0=censored)",
                ),
            },
            "WHAS500": {
                "age": (
                    "Age",
                    "Numerical (Years)",
                    "Patient age at hospital admission",
                ),
                "gender": ("Gender", "Binary (0=Male, 1=Female)", "Patient sex/gender"),
                "hr": (
                    "Heart Rate",
                    "Numerical (bpm)",
                    "Initial heart rate at admission",
                ),
                ...
                "lenfol": (
                    "Follow-up Length",
                    "Numerical (Days)",
                    "Total follow-up time from admission",
                ),
                "fstat": (
                    "Vital Status",
                    "Binary Target (0/1)",
                    "Final status (1=dead, 0=censored/alive)",
                ),
            },
            "METABRIC": {
                "age": ("Age", "Numerical (Years)", "Patient age at diagnosis"),
                ...
                "hormone_therapy": (
                    "Hormone Therapy",
                    "Binary (0/1)",
                    "Hormone therapy treatment received",
                ),
                "PAM50Subtype": (
                    "PAM50 Subtype",
                    "Categorical",
                    "Molecular subtype (Basal, Her2, LumA, LumB, Normal)",
                ),
                "duration": (
    ...
    def categorical_analysis(self):
        ...
        for col in self.headers:
            ...
            is_cat = (
                col in self.user_categorical_cols
                or any(isinstance(v, str) for v in vals)
                or (len(set(vals)) <= 10 and col != self.time_col)
            )
            if not is_cat:
                continue
            ...
            imbalance_ratio = (
                round(sorted_cnts[0] / sorted_cnts[-1], 2) if sorted_cnts[-1] > 0 else 0
            )
    ...
    def survival_target_analysis(self):
        ...
        med_t = (
            s_times[n_total // 2]
            if n_total % 2 == 1
            else (s_times[n_total // 2 - 1] + s_times[n_total // 2]) / 2.0
        )
        ...
        return {
            ...
            "survival_time": {
                ...
                "std": round(
                    math.sqrt(sum((x - mean_t) ** 2 for x in times) / (n_total - 1)), 2
                ),
            },
        }
    ...
    def kaplan_meier_analysis(self, stratify_col=None):
        ...
        if stratify_col is None:
            ...
            unique_strats = sorted(set(strat_vals))
            strat_results = {}
            for s_val in unique_strats:
                t_sub = [times[i] for i in range(len(times)) if strat_vals[i] == s_val]
                e_sub = [
                    events[i] for i in range(len(events)) if strat_vals[i] == s_val
                ]
                if t_sub:
    ...
    def correlation_analysis(self):
        ...
        return {"numerical_cols": num_cols, "pearson": pearson, "spearman": spearman}
    ...
    def outlier_detection(self):
        ...
        for col in self.headers:
            if col in self.user_categorical_cols or col in (self.event_col,):
                continue
            vals = [
                float(x) for x in self.columns_data[col] if isinstance(x, (int, float))
            ]
            if not vals:
                continue
    ...
    def clinical_insights(self):
        ...
        if self.dataset_name == "GBSG2":
            ...
            self.kaplan_meier_analysis("menostat")
            insights = {
            ...
            self.kaplan_meier_analysis("gender")
            self.kaplan_meier_analysis("chf")
            insights = {
                "q1_mortality_factors": "Congestive Heart Failure (chf=1) and Cardiogenic Shock (sho=1) are the strongest predictors of mortality post-MI. Patients with CHF have significantly lower survival probability.",
            ...
            self.kaplan_meier_analysis("PAM50Subtype")
            insights = {
    ...
    def _pearson_calc(self, v1, v2):
        ...
        den = math.sqrt(
            sum((v1[i] - m1) ** 2 for i in range(n))
            * sum((v2[i] - m2) ** 2 for i in range(n))
        )
        return num / (den + 1e-9)
