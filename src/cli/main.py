...
def main():
    parser = argparse.ArgumentParser(
        description="Bayesian Cox Survival Analysis Command Line Interface"
    )
    parser.add_argument(
        "--config", type=str, help="Path to experiment configuration YAML file"
    )
    parser.add_argument(
        "--dataset", type=str, help="Dataset name (gbsg2, metabric, whas500)"
    )
    parser.add_argument(
        "--model", type=str, default="bayesian_cox", help="Model type to train"
    )
