"""
Master Execution Script for Phase 7 — Random Survival Forests (RSF).
...
computes permutation feature importances (VIMP), compares with Cox PH baseline,
...
def draw_rsf_survival_plot(eval_times, surv_curves, dataset_name, filepath):
    ...
    draw.text((img_w // 2 - 200, 20), f"Random Survival Forest Curves — {dataset_name.upper()}", fill=(30, 41, 59))
    ...
    colors = [(16, 185, 129), (37, 99, 235), (220, 38, 38), (217, 119, 6), (147, 51, 234)]
...
def draw_vimp_bar_chart(df_vimp, dataset_name, filepath):
    ...
    draw.text((img_w // 2 - 220, 20), f"RSF Feature Importance (VIMP) — {dataset_name.upper()}", fill=(30, 41, 59))
    draw.text((img_w // 2 - 60, img_h - 35), "Drop in C-Index (VIMP)", fill=(51, 65, 85))
    ...
    for idx, row in df_vimp.iterrows():
        ...
        draw.rectangle([(pad_left, y_top), (pad_left + bar_w, y_bot)], fill=(16, 185, 129))
        draw.text((pad_left + bar_w + 10, y_top + 2), f"{vimp_val:.4f}", fill=(71, 85, 105))

    img.save(filepath)

def main():
    print("=" * 60)
    print("STARTING PHASE 7 — RANDOM SURVIVAL FORESTS (RSF)")
    print("=" * 60)
    ...
    for dname in datasets:
        print(f"\n[+] Training Random Survival Forest on Dataset: '{dname.upper()}'")
        ...
        # 1. Fit Random Survival Forest Model
        ...
        rsf_model.fit(X_train, y_train_time, y_train_event)

        vimp_df = rsf_model.get_summary()
        print("\n    Permutation Feature Importances (VIMP):")
        print(vimp_df.to_string(index=False))

        # 2. Test Evaluation
        eval_times = np.percentile(y_test_time, [25, 50, 75])
        test_risk = rsf_model.predict_risk(X_test)

        def surv_fn(times):
            return rsf_model.predict_survival(X_test, times)
        ...
        # 4. Compare with Cox PH Baseline
