...
def draw_posterior_hr_forest_plot(summary_df, dataset_name, filepath):
    ...
    draw.text((img_w // 2 - 220, 20), f"Bayesian Posterior Hazard Ratios — {dataset_name.upper()}", fill=(30, 41, 59))
    draw.text((img_w // 2 - 80, img_h - 35), "Hazard Ratio (Posterior 95% Credible Interval)", fill=(51, 65, 85))
    ...
    x_null = hr_to_x(1.0)
    draw.line([(x_null, pad_top), (x_null, img_h - pad_bot)], fill=(220, 38, 38), width=2)
    ...
    for hr_val in [0.5, 1.0, 1.5, 2.0, 2.5]:
        if min_hr <= hr_val <= max_hr:
            ...
            draw.line([(x_pos, pad_top), (x_pos, img_h - pad_bot)], fill=(241, 245, 249), width=1)
            draw.text((x_pos - 10, img_h - pad_bot + 10), f"{hr_val:.1f}", fill=(100, 116, 139))
    ...
    for idx, row in summary_df.iterrows():
        ...
        draw.ellipse([(x_mean - 5, y_pos - 5), (x_mean + 5, y_pos + 5)], fill=(147, 51, 234))
        draw.text((x_up + 10, y_pos - 8), f"{mean_hr:.2f} [{low_hr:.2f}, {up_hr:.2f}]", fill=(51, 65, 85))

    img.save(filepath)


def draw_bayesian_credible_survival_plot(eval_times, surv_mean, surv_low, surv_up, dataset_name, filepath):
    ...
    draw.text((img_w // 2 - 240, 20), f"Bayesian Survival & 95% Credible Band — {dataset_name.upper()}", fill=(30, 41, 59))
    ...
    leg_y = pad_top + 20
    draw.line([(leg_x, leg_y + 5), (leg_x + 20, leg_y + 5)], fill=(79, 70, 229), width=3)
    draw.text((leg_x + 30, leg_y), "Posterior Mean S(t)", fill=(30, 41, 59))

    draw.line([(leg_x, leg_y + 30), (leg_x + 20, leg_y + 30)], fill=(199, 210, 254), width=2)
...
def main():
    ...
    cox_results = json.load(open(cox_results_path, "r")) if os.path.exists(cox_results_path) else {}
    rsf_results = json.load(open(rsf_results_path, "r")) if os.path.exists(rsf_results_path) else {}
    deepsurv_results = json.load(open(deepsurv_results_path, "r")) if os.path.exists(deepsurv_results_path) else {}
    ...
    for dname in datasets:
        ...
        print(summary_df.head(5)[["feature", "exp(coef) HR Mean", "95% Credible Lower", "95% Credible Upper", "Prob(HR > 1)"]].to_string(index=False))
        ...
        def model_trainer(df_tr, df_v):
            ...
            m = BayesianCoxModel(n_intervals=6, inference_method="advi", n_advi_iterations=600, draws=250, random_state=42)
        ...
        ds_c = deepsurv_results.get(dname, {}).get("test_eval", {}).get("c_index", "N/A")
        ...
        hr_plot_path = os.path.join(FIGURES_DIR, f"bayesian_cox_{dname}_posterior_hr.png")
        ...
        s_mean, s_low, s_up = bayes_model.predict_survival_with_credible_intervals(med_sample, t_grid)

        surv_plot_path = os.path.join(FIGURES_DIR, f"bayesian_cox_{dname}_credible_survival.png")
        draw_bayesian_credible_survival_plot(t_grid, s_mean[0], s_low[0], s_up[0], dname, surv_plot_path)
