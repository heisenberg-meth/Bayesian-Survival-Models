...
def draw_training_curve_plot(loss_history, dataset_name, filepath):
    ...
    draw.text((img_w // 2 - 220, 20), f"DeepSurv Training Loss Curve — {dataset_name.upper()}", fill=(30, 41, 59))
    draw.text((img_w // 2 - 50, img_h - 35), "Optimization Iteration", fill=(51, 65, 85))
...
def draw_deepsurv_survival_plot(eval_times, surv_curves, dataset_name, filepath):
    ...
    draw.text((img_w // 2 - 200, 20), f"DeepSurv Neural Curves — {dataset_name.upper()}", fill=(30, 41, 59))
    ...
    colors = [(147, 51, 234), (37, 99, 235), (220, 38, 38), (16, 185, 129), (217, 119, 6)]
...
def main():
    ...
    for dname in datasets:
        ...
        def model_trainer(df_tr, df_v):
            ...
            m = DeepSurvModel(hidden_dims=[32, 16], l2_reg=1e-3, random_state=42, max_iter=200)
        ...
        # 5. Generate Figures
        loss_curve_path = os.path.join(FIGURES_DIR, f"deepsurv_{dname}_training_curves.png")
        draw_training_curve_plot(deepsurv_model.training_loss_history, dname, loss_curve_path)
