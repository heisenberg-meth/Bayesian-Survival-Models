...
datasets_config = {
    ...
    "METABRIC": {
        ...
        "time": "duration",
        "event": "event",
        "categorical": ["tumour_stage", "chemotherapy", "hormone_therapy", "PAM50Subtype"],
...
def main():
    ...
    for name, cfg in datasets_config.items():
        ...
        target_vals = [float(x) for x in analyzer.columns_data[cfg["time"]] if x is not None]
...
def generate_markdown_report(results):
    lines = []
    lines.append("# Phase 3 — Exploratory Data Analysis & Dataset Understanding Report\n")
    lines.append("**Project**: Bayesian Cox Proportional Hazards & Deep Survival Models\n")
    ...
    for name, data in results.items():
        ...
        lines.append(f"- **Duplicate Rows**: {integ['duplicate_rows']} ({integ['duplicate_pct']}%)\n")
        
        # 2. Data Dictionary
        lines.append("### Step 3.2 — Data Dictionary\n")
        lines.append("| Feature | Clinical Description | Data Type | Units / Range | Missing | Unique |")
        lines.append("| ------- | -------------------- | --------- | ------------- | ------- | ------ |")
        for fd in data["feature_dictionary"]:
            lines.append(f"| `{fd['feature']}` | {fd['label']} — {fd['description']} | {fd['type']} | N/A | {fd['missing']} | {fd['unique_values']} |")
        lines.append("\n")
        
        # 3. Missing & Duplicates
        lines.append("### Steps 3.3 & 3.4 — Missing Values & Duplicate Analysis\n")
        miss = data["missing"]
        lines.append(f"- **Total Missing Cells**: {miss['total_missing']} ({miss['total_missing_pct']}%)\n")
        
        # 4. Numerical Analysis
        lines.append("### Step 3.5 — Numerical Feature Descriptive Statistics\n")
        lines.append("| Feature | Mean | Median | Mode | Std Dev | Min | Max | Q1 | Q3 | IQR | Skewness | Kurtosis |")
        lines.append("| ------- | ---- | ------ | ---- | ------- | --- | --- | -- | -- | --- | -------- | -------- |")
        for feat, ns in data["numerical_stats"].items():
            lines.append(f"| `{feat}` | {ns['mean']} | {ns['median']} | {ns['mode']} | {ns['std']} | {ns['min']} | {ns['max']} | {ns['q1']} | {ns['q3']} | {ns['iqr']} | {ns['skewness']} | {ns['kurtosis']} |")
        lines.append("\n")
        
        # 5. Categorical Analysis
        lines.append("### Step 3.6 — Categorical Feature Analysis\n")
        for feat, cs in data["categorical_stats"].items():
            lines.append(f"#### Feature: `{feat}` (Imbalance Ratio: {cs['imbalance_ratio']}:1)")
            for cat_k, cat_v in cs["frequencies"].items():
                lines.append(f"  - `{cat_k}`: {cat_v['count']} records ({cat_v['percentage']}%)")
        lines.append("\n")
        ...
        lines.append(f"- **Survival Time Range**: {surv['survival_time']['min']} to {surv['survival_time']['max']}")
        ...
        lines.append(f"- **Overall Median Survival Time**: `{km_ov['median_survival_time']}`")
        lines.append(f"- **Final Horizon Survival Probability**: `{km_ov['final_survival_prob']:.4f}`\n")
        ...
        for strat_k, strat_v in data["kaplan_meier_stratified"].items():
            ...
            for grp_k, grp_v in strat_v.items():
                lines.append(f"  - Group `{grp_k}`: Median Survival = `{grp_v['median_survival_time']}`, Final S(t) = `{grp_v['final_survival_prob']:.4f}`")
            lines.append("")
            
        # 8. Outliers & Distribution
        lines.append("### Steps 3.10 & 3.11 — Outlier Detection & Feature Distributions\n")
        lines.append("| Feature | IQR Outliers | Z-Score Outliers | Distribution Shape | Preprocessing Recommendation |")
        lines.append("| ------- | ------------ | ---------------- | ------------------ | ---------------------------- |")
        for feat, out_v in data["outliers"].items():
            dist_v = data["distributions"].get(feat, {})
            lines.append(f"| `{feat}` | {out_v['iqr_outliers_count']} ({out_v['iqr_outliers_pct']}%) | {out_v['zscore_outliers_count']} ({out_v['zscore_outliers_pct']}%) | {dist_v.get('distribution_type', 'N/A')} | {dist_v.get('preprocessing_recommendation', 'N/A')} |")
        lines.append("\n")
